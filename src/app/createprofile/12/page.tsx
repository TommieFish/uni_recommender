...
export default function EnterDistance()
{
  const {distanceFromHome,setDistanceFromHome}  = useProfileForm();
  const[error,setError] = useState("");
  const router = useRouter();

  const isValid = distanceFromHome > 0;
  ...
  function Next()
  {
    ...
    else router.push("/createprofile/13");
  }

  function Back()
  {
    router.push("/createprofile/11");
  }

  return (
    <div className="flex flex-col items-center text-center space-y-6">
      <h1 className="font-bold text-3xl text-blue-700">What is the average distance you want a university to be from your home?</h1>
      <p className = "text-sm text-gray-500">In Kilometers (km)</p>
      ...
      <input
        type="number"
        value={distanceFromHome ===0 ? "" : distanceFromHome.toString()}
        max={800}
        onChange={(budget) => {
          const value = budget.target.value;
          const numValue = parseInt(value, 10); //base 10
          const clampedValue = isNaN(numValue) ? 0 : Math.min(numValue, 800); //guards against edge case (e.g: using dev console)
          setDistanceFromHome(isNaN(clampedValue) ? 0 : clampedValue);
          setError("")}
        }
        placeholder = "max 800km"
